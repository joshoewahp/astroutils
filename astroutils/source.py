...
class SelavyCatalogue:
    ...
    def from_params(cls, epoch: str, stokes: str, tiletype: str, fields: Strset='', sbids: Strset='', is_name: bool=False):
        ...
        for field, sbid in zip(fields, sbids):
            ...
            if len(files) == 0:
                files = list(selavypath.glob(f'*[._]{sbid}*{field}[AB.].*components.xml'))

            # and finally early RACS text catalogues
            if len(files) == 0:
                files = list(selavypath.glob(f'*[._]{sbid}*{field}[AB.].*components.txt'))
...
def force_measure_flux(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Image does not contain position <{position.ra:.2f},{position.dec:.2f}>")
            return None, None
...
def measure_epoch_flux(
    ...
    for _, field in fields.iterrows():
        ...
        _, header = get_image_from_survey_params(epoch, fieldname, stokes, ttype, load=False)
        ...
        if component is None:
            logger.debug(f"{epoch['name']} {fieldname} - no selavy component, performing forced fit")

            image_file = list(Path(epoch[f'image_path_{stokes}_{ttype[0]}']).glob(f'*{fieldname}*.fits'))[0]
