...
def get_surveys() -> pd.DataFrame:
    ...
    if system == "ada.physics.usyd.edu.au":
        ...
        surveys["data_path"] = surveys.ada_root
        logger.error(
            f"Data paths unknown for hostname {system}. Defaulting to ada paths."
        )

    # Populate all relevant data paths
    def update(row):
        row["image_path_i_C"] = (
            (row.data_path + "COMBINED/STOKESI_IMAGES/") if row.combined_I else ""
        )
        row["selavy_path_i_C"] = (
            (row.data_path + "COMBINED/STOKESI_SELAVY/") if row.combined_I else ""
        )
        row["image_path_v_C"] = (
            (row.data_path + "COMBINED/STOKESV_IMAGES/") if row.combined_V else ""
        )
        row["selavy_path_v_C"] = (
            (row.data_path + "COMBINED/STOKESV_SELAVY/") if row.combined_V else ""
        )
        row["image_path_i_T"] = (
            (row.data_path + "TILES/STOKESI_IMAGES/") if row.tiles_I else ""
        )
        row["selavy_path_i_T"] = (
            (row.data_path + "TILES/STOKESI_SELAVY/") if row.tiles_I else ""
        )
        row["image_path_v_T"] = (
            (row.data_path + "TILES/STOKESV_IMAGES/") if row.tiles_V else ""
        )
        row["selavy_path_v_T"] = (
            (row.data_path + "TILES/STOKESV_SELAVY/") if row.tiles_V else ""
        )
...
>>>>>>> 8434e1a (updated dependencies)
    ...
    image_path = list(
        Path(epoch[f"image_path_{stokes}_{tiletype[0]}"]).glob(f"*{field}*.fits")
    )[0]
...
def parse_image_filenames_in_dir(path: Path, stokes: str) -> list[Path]:
    ...
    image_paths = [
        p for p in path.iterdir() if any(p.match(pattern) for pattern in patterns)
    ]
...
def build_field_csv(epoch: str, tiletype: str = "TILES") -> pd.DataFrame:
    ...
    i_paths = parse_image_filenames_in_dir(
        Path(survey[f"image_path_i_{tiletype[0]}"]), stokes="i"
    )
    v_paths = parse_image_filenames_in_dir(
        Path(survey[f"image_path_v_{tiletype[0]}"]), stokes="v"
    )
    ...
    for i_path in i_paths:
        ...
        if field == str_path:
            raise FITSException(
                f"Could not parse field name or SBID from image at {str_path}"
            )
        ...
        central_coords = [[size_x / 2.0, size_y / 2.0]]
...
def build_vlass_field_csv(epoch: str) -> pd.DataFrame:
    ...
    fields = [
        Path(f)
        for f in glob.glob(f"{survey.data_path}/**/*.subim.fits", recursive=True)
    ]
